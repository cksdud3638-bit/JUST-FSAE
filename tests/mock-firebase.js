...
(() => {
  const key='just-inspection-test-v2';
  let root=JSON.parse(localStorage.getItem(key) || 'null') || {
    inspection:{br13:'pass',ae03:'pass',vc01:'pass'},inspectionMeta:{br13:{assignee:'검증 담당',failReason:'기존 메모 보존'}},
  ...
  window.firebase={initializeApp(){},database:()=>({ref:path=>({
    ...
    set:async value=>set(path,value), update:async updates=>{
      for(const [relative,value] of Object.entries(updates)){
        const keys=(path+'/'+relative).split('/').slice(1);let node=root;
